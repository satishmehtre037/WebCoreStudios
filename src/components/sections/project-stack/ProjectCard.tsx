  ...
  ({ project, index, onViewCaseStudy, onOpenLiveWebsite, className, style }, ref) => {
    return (
      <Card
        ...
        variant="elevated"
        className={cn(
          "group relative w-full rounded-3xl border border-border/80 bg-surface/95 overflow-hidden",
          "shadow-xl hover:shadow-glow hover:border-primary/40",
          "transition-all duration-500 ease-out hover:scale-[1.01]",
          "p-6 md:p-10 flex flex-col lg:flex-row gap-8 lg:gap-12 items-stretch select-none",
          className
        )}
      >
        ...
        <div className="flex flex-col justify-between flex-1 space-y-6 z-10">
          {/* Header Tag & Status */}
          <div className="space-y-4">
            ...
            {/* Product Title */}
            <Heading level="h2" className="text-h2 md:text-h1 tracking-tight text-foreground group-hover:text-primary transition-colors">
              {project.title}
            </Heading>

            {/* Short Description */}
            <p className="text-body-md text-foreground/80 leading-relaxed max-w-xl">
              {project.description}
            </p>
          </div>
        ...
        <div className="flex-1 relative min-h-[300px] lg:min-h-full rounded-2xl border border-border/60 bg-background/50 overflow-hidden flex items-center justify-center z-10">
